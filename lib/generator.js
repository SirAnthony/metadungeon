...
G.start = function(dungeon, levels){
    ...
    var room = dungeon.start = new Room(coords);
    dungeon.add(room);
    levels.push([]);
    levels[0].push(room);
};
...
G.finish = function(dungeon, levels){
    ...
    var goal = dungeon.goal = goals[gnum];
    var boss = dungeon.boss = goal.parent;
...
function freeEdged(rooms, dungeon){
    ...
    var valid = null;
    var validate = function(d, coords){
        if (!valid && dungeon.valid(coords) &&
            !dungeon.get(coords))
            valid = {room: room, next: coords};
    };
    for (var r=0; r<srooms.length; ++r){
        var room = srooms[r];
        room.mapAround(validate);
        if (valid)
            return valid;
    }
}
